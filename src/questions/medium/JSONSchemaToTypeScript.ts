...
type JSONSchema2TS<T> = T extends { enum: infer E }
          ...
          : T extends { type: 'array' }
            ...
            : T extends { type: 'object'; properties: infer Props }
              ? Props extends Record<string, unknown>
                ? ObjectFromProps<Props, T>
                : {}
              : T extends { type: 'object' }
                ? Record<string, unknown>
                : unknown;

type ObjectFromProps<Props, Schema> = keyof Props extends never
  ? {}
  : Schema extends { required: infer Required }
    ? Required extends readonly string[]
      ? Prettify<
          {
            [K in keyof Props as K extends Required[number]
              ? K
              : never]: JSONSchema2TS<Props[K]>;
          } & {
            [K in keyof Props as K extends Required[number]
              ? never
              : K]?: JSONSchema2TS<Props[K]>;
          }
        >
